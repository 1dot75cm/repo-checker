...
def get_page(url, url_type):
    ...
    if url_type == "2":
        ...
        return urllib.request.urlopen(url).read().decode('gb2312').encode('utf-8')
    else:
...
def get_date(page, url_type):
    ...
    if url_type in ["4", "6", "14"]:
        ...
        page = page.split(b"tragtor")[4]
        pattern = b'time.>(.*[0-9]{4})'
        time_format = "%a %b %d %H:%M:%S %Y"
    elif url_type == "7":
        if page.splitlines()[-3].find(b"info") > -1:
        ...
        page = json.loads(page.decode())['PCC'][0]['date'].encode()
        pattern = b'(.*)'

    try:
        d = re.search(pattern, page, re.M).group(1).decode()
...
def get_commit(page, url_type):
    ...
    try:
        c = re.search(pattern, page, re.M).group(1).decode()
    except (AttributeError, TypeError):
        c = "None"
